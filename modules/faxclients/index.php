...
function _moduleContent(&$smarty, $module_name)
{
    ...
    $smarty->assign(array(
        ...
        'EMAIL_RELAY_MSG'   =>  _tr('These IPs are allowed to send faxes through Issabel.  You must insert one IP per row.  We recommend keeping localhost and 127.0.0.1  in the configuration because some processes could need them.'),
    ...
    $output = NULL; $retval = NULL;
    exec('/usr/bin/issabel-helper faxconfig --getfaxhosts 2>&1', $output, $retval);
    if ($retval != 0) {
    	$smarty->assign(array(
            'mb_title'      =>  _tr('Error'),
